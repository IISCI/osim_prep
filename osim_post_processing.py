...
import logging
from csv import reader  # to replace R 'read.table'
...
# library("methods")
# logReset();
log = logging.getLogger("osim-logger")
# basicConfig('DEBUG');
# addHandler(writeToConsole);
# addHandler(writeToFile, file=paste("output-",System$getHostname(),'-',Sys.Date(),".log",sep=''), level='DEBUG');

# Путь к файлу с моделями?
# path = '/Users/proton/Google\ Диск/Science/OpensimModeling/Storage/model/' # R
path = '/Users/proton/Google\ Диск/Science/OpensimModeling/Storage/model/'
...
if (save_to_file)
{
    png_file = paste(path, '/data/', athlete, '/', experiment,
                     '_plot_1_center_of_mass_delta_raw.png', sep="")
...
movement_frame = min(
    frames_array[abs(center_of_mass_Y_delta_smoothed) > movement_tolerance]) - 2
...
lowest_frame = min(
    frames_array[center_of_mass_Y_smoothed <= min(center_of_mass_Y_smoothed)])
...
normalize_file(pos_source, paste(path, '/data/', athlete, '/',
               experiment, '_normalized_1_pos.csv', sep=''))
normalize_file(vel_source, paste(path, '/data/', athlete, '/',
               experiment, '_normalized_2_vel.csv', sep=''))
normalize_file(acc_source, paste(path, '/data/', athlete, '/',
               experiment, '_normalized_3_acc.csv', sep=''))
normalize_file(body_forces_source, paste(path, '/data/', athlete,
               '/', experiment, '_normalized_4_body_forces.csv', sep=''))
normalize_file(inverse_dynamics_source, paste(path, '/data/', athlete,
               '/', experiment, '_normalized_5_inverse_dynamics.csv', sep=''))
normalize_file(mot_source, paste(path, '/data/', athlete, '/',
               experiment, '_normalized_6_mot.csv', sep=''))
normalize_force_file(ext_forces_source, paste(
    path, '/data/', athlete, '/', experiment, '_normalized_7_ext_forces.csv', sep=''))
...
get_segment_mass < - function(segment)
{
    ...
    seg = getNodeSet(model_xml, paste(
        "//Body[@name='", segment, "']/mass", sep=''))
...
pos_smoothed < - read.table(paste(path, '/data/', athlete, '/', experiment,
                            '_normalized_1_pos.csv', sep=''), header=TRUE, sep=";", dec=',')
vel_smoothed < - read.table(paste(path, '/data/', athlete, '/', experiment,
                            '_normalized_2_vel.csv', sep=''), header=TRUE, sep=";", dec=',')
acc_smoothed < - read.table(paste(path, '/data/', athlete, '/', experiment,
                            '_normalized_3_acc.csv', sep=''), header=TRUE, sep=";", dec=',')
body_forces_smoothed < - read.table(paste(path, '/data/', athlete, '/', experiment,
                                    '_normalized_4_body_forces.csv', sep=''), header=TRUE, sep=";", dec=',')
inverse_dynamics_smoothed < - read.table(paste(path, '/data/', athlete, '/', experiment,
                                         '_normalized_5_inverse_dynamics.csv', sep=''), header=TRUE, sep=";", dec=',')
mot_smoothed < - read.table(paste(path, '/data/', athlete, '/', experiment,
                            '_normalized_6_mot.csv', sep=''), header=TRUE, sep=";", dec=',')
ext_forces_smoothed < - read.table(paste(path, '/data/', athlete, '/', experiment,
                                   '_normalized_7_ext_forces.csv', sep=''), header=TRUE, sep=";", dec=',')

# 1 frame - beginning of ammortization phase, 400 - end of push-off
phase_split = round((lowest_frame - movement_frame) /
                    (eject_frame - movement_frame) * 400, digits=0)
...
amort_stiffness_K_sum = max_sum_of_left_and_right_force_Y / \
    (amort_com_max - amort_com_min)
amort_stiffness_K_avg = max_avg_of_left_and_right_force_Y / \
    (amort_com_max - amort_com_min)
...
I_foot = ((get_segment_mass("toes_r") + get_segment_mass("calcn_r") + get_segment_mass("talus_r"))/660.49) + \
    ((get_segment_mass("toes_r") + get_segment_mass("calcn_r") +
     get_segment_mass("talus_r"))/4716100)
I_shank = ((get_segment_mass("tibia_r"))/789.61) + \
    ((get_segment_mass("tibia_r"))/278784)
I_thigh = ((get_segment_mass("femur_r"))/712.89) + \
    ((get_segment_mass("femur_r"))/291600)
...
Power_sum = abs(Power_knee_right) + \
    abs(Power_ankle_right) + abs(Power_hip_right)
...
power_and_moments_file = paste(path, '/data/', athlete, '/',
                               experiment, '_normalized_8_RCALC_power_and_moments.csv', sep='')
columns = c(
    'Ekin1',
    'Ekin_foot_right',
    'Ekin_shank_right',
...
summary_file = paste(path, '/data/', athlete, '/', experiment,
                     '_normalized_9_RCALC_summary.csv', sep='')
...
lines(x=c(-10000, 10000), y=c(0, 0), col='black')
title(main=paste(
    "Jump Phases Breakdown (Using smoothed Center of Mass + Force Reactions)", sep=''))
title(ylab="Delta (m)")
title(xlab=paste("Frames", sep=""))
...
lines(x=c(eject_frame, eject_frame), y=c(
    bottom, top), lty=2, lwd=2, col="purple")
lines(x=c(movement_frame, movement_frame), y=c(
    bottom, top*0.6), lty=2, lwd=2, col="orange")
lines(x=c(lowest_frame, lowest_frame), y=c(
    bottom, top), lty=2, lwd=2, col="brown")
lines(x=c(highest_frame, highest_frame), y=c(
    bottom, top), lty=2, lwd=2, col="darkgreen")
if(save_to_file) dev.off()
...
{
    png_file = paste(path, '/data/', athlete, '/', experiment,
                     '_plot_3_knee_moment_vs_angular_velocity.png', sep="")
    png(png_file, width=1600, height=1000, units="px", res=106)
}
plot(x=knee_angle_r_moment_smoothed, y=angular_velocity_knee_Oz_smoothed,
     ann=FALSE, type="l", col="black", lwd=2)
points(x=knee_angle_r_moment_smoothed[1],
       y=angular_velocity_knee_Oz_smoothed[1], pch=17, col="grey", cex=3)
points(x=knee_angle_r_moment_smoothed[movement_frame],
       y=angular_velocity_knee_Oz_smoothed[movement_frame], pch=18, col="orange", cex=3)
points(x=knee_angle_r_moment_smoothed[lowest_frame],
       y=angular_velocity_knee_Oz_smoothed[lowest_frame], pch=18, col="brown", cex=3)
points(x=knee_angle_r_moment_smoothed[eject_frame],
       y=angular_velocity_knee_Oz_smoothed[eject_frame], pch=18, col="purple", cex=3)
points(x=knee_angle_r_moment_smoothed[frames],
       y=angular_velocity_knee_Oz_smoothed[frames], pch=19, col="red", cex=3)
points(x=knee_angle_r_moment_smoothed[highest_frame],
       y=angular_velocity_knee_Oz_smoothed[highest_frame], pch=17, col="darkgreen", cex=3)
